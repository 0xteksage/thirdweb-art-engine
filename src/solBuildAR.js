...
const buildDir = `${basePath}/solbuildARFiles`;
...
const {
  ...
  gif,
} = require(`${basePath}/src/solconfig.js`);
...
const saveImage = (_editionCount) => {
  fs.writeFileSync(`${buildDir}/assets/${_editionCount}.png`, canvas.toBuffer("image/png"));
};
...
const addAttributes = (_element) => {
  ...
  attributesList.push({
    ...
    value: `${
      isLayerNameFileNameAsIs ? selectedElement.name : capitalizeEachWord(cleanedElementName)
    }`,
  });
};
...
const saveMetaDataSingleFile = (_editionCount) => {
  ...
  debugLogs
    ...
    : null;
  fs.writeFileSync(`${buildDir}/assets/${_editionCount}.json`, JSON.stringify(metadata, null, 2));
};
...
const startGeneration = async () => {
  const { symbol } = require("./solconfig.js");
  ...
  console.log(
    `[Success]: Your NFTs have been generated. You can explore them in the "build" folder.`
  );
};
